...
export function LearningSidebar({ open, words, notes, resource, onClose, onSaveNote, onRemoveWord }: LearningSidebarProps) {
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        {tab === "notes" && <div className="space-y-3"><textarea defaultValue={notes[0]?.content || ""} onChange={(event) => onSaveNote({ id: notes[0]?.id || "personal", content: event.target.value, createdAt: new Date().toISOString() })} placeholder="Your personal notes auto-save as you type..." rows={12} className="w-full resize-none rounded-lg border border-[#29303c] bg-[#0c1017] p-3 text-sm leading-relaxed text-stone-200 outline-none focus:border-amber-500" /><p className="text-[10px] text-stone-600">Auto-saved locally for this student.</p></div>}
        {tab === "resources" && <div className="space-y-3">{resource ? <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 text-sm leading-relaxed text-stone-300">{resource}</div> : <p className="text-sm text-stone-500">Your instructor has not prescribed a resource yet.</p>}<div className="rounded-lg border border-[#29303c] bg-[#0c1017] p-4"><p className="text-xs font-semibold text-amber-300">Study Hub</p><p className="mt-2 text-xs leading-relaxed text-stone-500">Instructor recommendations and lesson resources will appear here.</p></div></div>}
