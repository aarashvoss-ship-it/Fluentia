...
export function ChatWidget({ messages, onSend }: ChatWidgetProps) {
  ...
  function send(event: FormEvent) { event.preventDefault(); if (!text.trim()) return; onSend({ id: `${Date.now()}`, tab, text: text.trim(), sender: "user", timestamp: new Date().toISOString() }); setText(""); }
