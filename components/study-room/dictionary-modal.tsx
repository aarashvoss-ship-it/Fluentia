    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} role="dialog" aria-modal="true" aria-label="Dictionary lookup">
      <div className="w-full max-w-lg rounded-xl border border-[#394252] bg-[#171d28] p-5 text-[#e8e7e4] shadow-2xl">
        ...
        {entry && !loading && <div className="space-y-4 pt-4">
          <div><div className="flex items-center gap-2"><h3 className="font-[var(--font-fraunces)] text-2xl text-stone-100">{entry.word}</h3>{entry.partOfSpeech && <span className="text-xs italic text-amber-400">{entry.partOfSpeech}</span>}{entry.pronunciationUrl && <button type="button" onClick={() => { const a = new Audio(entry.pronunciationUrl!); void a.play(); }} aria-label="Play pronunciation" className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-amber-500/40 bg-[#0c1017] text-amber-400 hover:bg-amber-500/10"><Volume2 className="h-3.5 w-3.5" /></button>}</div>{entry.phonetic && <p className="mt-1 text-xs text-stone-500">{entry.phonetic}</p>}<p className="mt-2 text-sm leading-relaxed text-stone-300">{entry.definition}</p></div>
