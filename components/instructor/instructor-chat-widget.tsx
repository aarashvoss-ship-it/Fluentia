...
export function InstructorChatWidget({ activeStudent, students, instructorId, lessonContext }: InstructorChatWidgetProps) {
  ...
  const currentThreadStudentIdRef = useRef<string | null>(null);
  ...
  function playNotificationSound() {
    ...
    gain.gain.exponentialRampToValueAtTime(0.045, startTime + 0.015);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `instructor_id=eq.${instructorId}` }, (payload) => {
        ...
        const isIncoming = row.receiver_id === instructorId && row.sender_id !== instructorId;
        const isCurrentThread = tabRef.current === "active" && row.student_id === currentThreadStudentIdRef.current;
        if (isIncoming && !isCurrentThread) {
  ...
  useEffect(() => {
    tabRef.current = tab;
    currentThreadStudentIdRef.current = conversationStudent ? studentKey(conversationStudent) : null;
  ...
  function viewStudentThread(studentId: string) {
    ...
    currentThreadStudentIdRef.current = studentKey(students.find((student) => student.id === studentId) || activeStudent!);
  ...
  return (
    <div ref={containerRef} className="fixed bottom-6 right-6 z-40">
      <button type="button" onClick={() => open ? setOpen(false) : openChat()} aria-label={open ? "Close instructor chat" : "Open instructor chat"} className="relative flex h-12 w-12 items-center justify-center rounded-full bg-amber-500 text-slate-950 shadow-xl transition hover:bg-amber-400">
