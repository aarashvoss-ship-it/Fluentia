...
export async function fetchLessonState(slug: string, studentToken?: string): Promise<PublishedLessonState | null> {
  if (isSupabaseConfigured()) {
    try {
      ...
      if (lesson && studentId) {
        const { data: submission } = await supabase.from("student_submissions").select("*").eq("lesson_id", lesson.id).eq("student_id", studentId).eq("step_key", "lesson").order("updated_at", { ascending: false }).limit(1).maybeSingle();
