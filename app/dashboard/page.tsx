        ...
        <header style={{ backgroundImage: `linear-gradient(90deg, rgba(12,16,23,.96), rgba(12,16,23,.62)), url(${dashboardHeaderBanner})` }} className="relative flex min-h-[280px] w-full flex-col justify-end overflow-visible bg-slate-950 bg-cover bg-center p-8 md:min-h-[340px]">
          ...
          <div className="mt-3 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            ...
            <div className="absolute top-6 right-8 z-10 flex items-center gap-2">
              <button type="button" onClick={() => setDictionaryOpen(true)} aria-label="Open dictionary" className="h-9 px-3 flex items-center gap-2 rounded-lg bg-slate-800/80 border border-amber-500/50 text-xs font-medium text-amber-400 hover:border-amber-400 hover:bg-slate-800 transition-colors"><BookOpen className="w-4 h-4 shrink-0 text-amber-400" /></button>
              <button type="button" onClick={() => setSidebarOpen((open) => !open)} aria-expanded={sidebarOpen} aria-controls="learning-sidebar" className="h-9 px-3 flex items-center gap-2 rounded-lg bg-slate-800/80 border border-amber-500/50 text-xs font-medium text-amber-400 hover:border-amber-400 hover:bg-slate-800 transition-colors"><PanelRight className="w-4 h-4 shrink-0 text-amber-400" />Learning Hub</button>
