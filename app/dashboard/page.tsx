...
const AVATAR_PRESETS = [
  { id: "amber", label: "Amber", initials: "FS", className: "bg-amber-500 text-slate-950" },
  { id: "copper", label: "Copper", initials: "FV", className: "bg-orange-700 text-orange-50" },
  { id: "slate", label: "Slate", initials: "FL", className: "bg-slate-500 text-slate-950" },
] as const;
    ...
    <main className="min-h-screen bg-[#0c1017] text-[#e8e7e4] font-sans">
      <div className="mx-auto max-w-6xl px-6 py-10 md:py-14">
          ...
          <div className="mt-3 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative flex items-center gap-2 border-l border-[#29303c] pl-3" aria-label="Student profile">
                ...
                <button type="button" onClick={() => { setProfileOpen(true); setProfileEditing(true); }} aria-label="Profile settings" title="Profile settings" className="text-stone-500 transition hover:text-amber-300"><Settings2 className="h-4 w-4" /></button>
                {profileOpen && (
                  <div id="student-profile-flyout" className="absolute right-0 top-12 z-30 w-72 rounded-xl border border-[#394252] bg-[#171d28] p-4 text-left shadow-2xl">
                    ...
                    {profileEditing && <div className="space-y-4 border-t border-[#29303c] pt-3">
                      <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-400">Student Avatar</p><div className="mt-2 grid grid-cols-3 gap-2">{AVATAR_PRESETS.map((preset) => <button key={preset.id} type="button" onClick={() => { setAvatarPreset(preset.id); setCustomAvatarUrl(""); }} aria-label={`Use ${preset.label} avatar`} className={`flex flex-col items-center gap-1 rounded-md border p-2 text-[10px] text-stone-400 transition ${avatarPreset === preset.id && !avatarImage ? "border-amber-500 bg-amber-500/10 text-amber-300" : "border-[#394252] hover:border-amber-500/50"}`}><span className={`flex h-8 w-8 items-center justify-center rounded-full text-[10px] font-bold ${preset.className}`}>{preset.initials}</span>{preset.label}</button>)}</div><label className="mt-2 block text-xs text-stone-400">Custom Avatar URL<input value={customAvatarUrl} onChange={(event) => setCustomAvatarUrl(event.target.value)} placeholder="https://..." className="mt-1 w-full rounded-md border border-[#394252] bg-[#0c1017] p-2 text-xs text-stone-200" /></label></div>
                      <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-400">Dashboard Hero Banner</p><div className="mt-2 grid grid-cols-3 gap-2">{BANNER_PRESETS.map((preset) => <button key={preset.id} type="button" onClick={() => { setBannerPreset(preset.id); setCustomBannerUrl(""); }} className={`overflow-hidden rounded-md border text-left transition ${bannerPreset === preset.id && !customBannerUrl ? "border-amber-500" : "border-[#394252] hover:border-amber-500/50"}`}><img src={preset.image} alt="" className="h-10 w-full object-cover opacity-75" /><span className="block truncate px-1.5 py-1 text-[9px] text-stone-400">{preset.label}</span></button>)}</div><label className="mt-2 block text-xs text-stone-400">Custom Banner URL<input value={customBannerUrl} onChange={(event) => setCustomBannerUrl(event.target.value)} placeholder="https://..." className="mt-1 w-full rounded-md border border-[#394252] bg-[#0c1017] p-2 text-xs text-stone-200" /></label></div>
                      <label className="block text-xs text-stone-400">Level<select value={profileLevel} onChange={(event) => setProfileLevel(event.target.value)} className="mt-1 w-full rounded-md border border-[#394252] bg-[#0c1017] p-2 text-xs text-stone-200"><option>B1 Intermediate</option><option>B2 Upper Intermediate</option><option>C1 Advanced</option></select></label>
                      <label className="block text-xs text-stone-400">Learning goal<input value={profileGoal} onChange={(event) => setProfileGoal(event.target.value)} className="mt-1 w-full rounded-md border border-[#394252] bg-[#0c1017] p-2 text-xs text-stone-200" /></label>
                      <button type="button" onClick={() => { const profile = { ...activeStudent.profile, level: profileLevel, targetGoal: profileGoal }; setActiveStudent({ ...activeStudent, profile }); window.localStorage.setItem(`fluentia:profile:${token}`, JSON.stringify({ level: profileLevel, targetGoal: profileGoal, avatarPreset, customAvatarUrl, bannerPreset, customBannerUrl })); setProfileEditing(false); }} className="w-full rounded-md bg-amber-500 px-3 py-2 text-xs font-semibold text-slate-950 hover:bg-amber-400">Save settings</button>
                    </div>}
        ...
        {nextLesson && <section className="mt-6" aria-label="Continue learning">
          <Link href={getLessonHref(nextLesson.slug, getLessonStatus(lessonStates[nextLesson.slug]))} className="group relative block h-64 overflow-hidden rounded-xl border border-amber-500/30 bg-[#171d28] transition-colors hover:border-amber-400/70">
