        ...
        <header style={{ backgroundImage: `linear-gradient(90deg, rgba(12,16,23,.96), rgba(12,16,23,.62)), url(${dashboardHeaderBanner})` }} className="relative flex min-h-[280px] w-full flex-col justify-end overflow-visible bg-slate-950 bg-cover bg-center p-8 md:min-h-[340px]">
          ...
          <div className="mt-3 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            ...
            <div className="absolute top-6 right-8 z-10 flex items-center gap-2">
              <button type="button" onClick={() => setDictionaryOpen(true)} aria-label="Open dictionary" className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700/60 bg-slate-800/80 text-stone-400 transition hover:border-amber-500 hover:text-amber-300"><BookOpen className="h-4 w-4" /></button>
              <button type="button" onClick={() => setSidebarOpen((open) => !open)} aria-expanded={sidebarOpen} aria-controls="learning-sidebar" className={`flex h-9 items-center gap-1.5 rounded-lg border px-3 text-sm font-medium transition ${sidebarOpen ? "border-amber-500/70 bg-amber-500/10 text-amber-400" : "border-slate-700/60 bg-slate-800/80 text-amber-400 hover:border-amber-500"}`}><PanelRight className="h-3.5 w-3.5" />Learning Hub</button>
      ...
      <div className="mx-auto max-w-5xl px-4 py-8">
        ...
        <section className="grid gap-3 border-b border-[#202631] py-6 sm:grid-cols-3" aria-label="Student progress overview">
          <div className="relative rounded-xl border border-[#202631] bg-[#121721] p-4 md:col-start-3" aria-label="Student profile">
            ...
            {profileOpen && <div id="student-profile-flyout" className="absolute right-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] rounded-xl border border-[#394252] bg-[#171d28] p-4 text-left shadow-2xl">
              <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-400">Student profile</p><p className="mt-1 text-sm font-semibold text-stone-100">{displayName}</p></div><button type="button" onClick={() => setProfileOpen(false)} aria-label="Close profile" className="text-stone-500 hover:text-stone-200"><X className="h-4 w-4" /></button></div>
              <div className="mt-4 space-y-3 text-xs"><p className="text-stone-500">Progress <span className="float-right text-stone-200">{completedLessons} / {displayLessons.length} lessons</span></p><p className="text-stone-500">Learning goal <span className="float-right max-w-[10rem] truncate text-stone-200">{activeStudent.profile?.targetGoal || "Not set"}</span></p><label className="block text-stone-400">Custom Avatar URL<input value={customAvatarUrl} onChange={(event) => setCustomAvatarUrl(event.target.value)} placeholder="https://..." className="mt-1 w-full rounded-md border border-[#394252] bg-[#0c1017] p-2 text-xs text-stone-200" /></label><label className="block text-stone-400">Custom Banner URL<input value={customBannerUrl} onChange={(event) => setCustomBannerUrl(event.target.value)} placeholder="https://..." className="mt-1 w-full rounded-md border border-[#394252] bg-[#0c1017] p-2 text-xs text-stone-200" /></label></div>
              <button type="button" onClick={() => { const preferences = { avatarPreset, customAvatarUrl, bannerPreset, customBannerUrl }; window.localStorage.setItem(`fluentia:profile:${token}`, JSON.stringify(preferences)); window.dispatchEvent(new CustomEvent("fluentia:student-profile-updated", { detail: preferences })); setBannerLoadFailed(false); setProfileOpen(false); }} className="mt-4 w-full rounded-md bg-amber-500 px-3 py-2 text-xs font-semibold text-slate-950 hover:bg-amber-400">Save settings</button>
            </div>}
          </div>
